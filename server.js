...
// Persistência de jobs em arquivo para não perder ao reiniciar
...
function salvarJobs() {
  ...
  } catch(e) { console.error('Erro ao salvar jobs:', e.message); }
}
...
async function processarComAudioBuffer(jobId, audioBuffer) {
  ...
  try {
    ...
    await Promise.allSettled([enviarEmail(job, jobId, docxBuffer), enviarWhatsapp(job, jobId)]);
...
// EMAIL
async function enviarEmail(job, jobId, docxBuffer) {
